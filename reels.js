// YouTube API Backend Configuration
...
// Fetch farming reels from backend API
async function fetchFarmingReels() {
    try {
        console.log('Fetching farming reels from API...');
        showLoading(true);
        const response = await fetch(`${API_BASE_URL}/reels?limit=10`);
        const data = await response.json();
        console.log('API Response:', data);

        if (data.success && data.data && data.data.length > 0) {
            console.log('Using API data:', data.data.length, 'videos');
            allFarmingReels = data.data;
            filteredReels = [...allFarmingReels];
            renderReels(filteredReels);
        } else {
            console.log('API failed or returned no data - using fallback reels');
            loadFallbackReels();
        }
    } catch (error) {
